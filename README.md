...
I find this approach promissing because it is fast and scalable. Concerning speed, it beats the industry-standard 'shunting-yard' algorithm. As for scalability, it should be very simple to add new operators and rules. All the same, we can apply this algorithm/idea for quite different tasks. In a pretty much the same way, we can efficiently make AST trees, evaluate expressions, present them in  <i>post-order</i> or <i>pre-order</i> form, handle unary operators and functions, etc. The applied code is expected to be small, readable and fast, as a consequence of a simple and straightforward idea.
